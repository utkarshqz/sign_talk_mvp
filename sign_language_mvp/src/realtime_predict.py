...
model = GestureNet(30 * 21 * 2, len(labels))
model.load_state_dict(torch.load(os.path.join(MODEL_DIR, "gesture_model.pth")))
model.eval()
    ...
    if key == ord('s'):
        if not recording:
            ...
            if len(sequence) > 5:
                ...
                with torch.no_grad():
                    out = model(torch.tensor(seq, dtype=torch.float32))
