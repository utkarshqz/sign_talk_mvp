...
def temporal_normalize(seq, target_len=30):
    """
    Speed normalization:
    resample sequence so motion is evenly distributed in time
    """
    seq = np.array(seq)
    T = len(seq)
    if T == target_len:
    ...
    for i in idx:
        ...
        alpha = i - low
        out.append((1 - alpha) * seq[low] + alpha * seq[high])
    return np.array(out)
...
print("2-second sentence mode + SPEED NORMALIZATION | 'c' clear | 'q' quit")
...
while True:
    ...
    if capturing and (now - start_time) >= CAPTURE_SECONDS:
        ...
        if len(sequence) >= 10:
            ...
            seq = temporal_normalize(seq, TARGET_LENGTH)
            seq = seq.reshape(1, -1)
            ...
            if best_dist < threshold:
                if best_gesture == "me" and motion > ME_MOTION_REJECT_THRESHOLD:
                    prediction = "UNKNOWN"
                    print("Rejected 'me' due to motion")
                else:
    ...
    cv2.imshow("2-Second Sentence Recognition (Speed-Normalized)", frame)
...
# this work also if signs perform fastly :) , speed normalization implemented , 
#issue is arriving because noise generated after performing sign and moving hand out of the window